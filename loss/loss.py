...
def jacobian_loss(deformation_field):
    """
    计算形变场的雅可比损失函数

    参数：
        deformation_field (numpy数组): 形变场，形状为 (B, C, H, W, D)，
                                      B 表示 batch size，C 表示通道数，H、W 和 D 表示图像的高度、宽度和深度。

    返回：
        雅可比损失函数值
    """
    # 计算 x、y 和 z 方向上的形变场梯度
    df_dx, df_dy, df_dz = np.gradient(deformation_field, axis=(3, 2, 4))

    # 计算雅可比行列式的分量
    jacobian_det = df_dx * df_dy * df_dz

    # 计算雅可比损失函数，这里取平均值
    jacobian_loss_value = np.mean(np.abs(jacobian_det))

    return jacobian_loss_value
...
class NCC_vxm(torch.nn.Module):
    ...
    def forward(self, y_true, y_pred):
        ...
        print("cross: ", torch.mean(cross))
        print("I_var: ", torch.mean(I_var))
        print("J_var: ", torch.mean(J_var))
...
class MIND_loss(torch.nn.Module):
    ...
    def forward(self, y_pred, y_true):
        o = self.MINDSSC(y_pred).detach().cpu().numpy()
        del y_pred
        y = self.MINDSSC(y_true).detach().cpu().numpy()
        return torch.mean((o - y) ** 2)
...
class SSDloss(torch.nn.Module):
    ...
    def forward(self, y_true, y_pred):
        ...
        assert y_true.shape == y_pred.shape, "Masks should have the same shape."

        # Compute SSD distance on GPU
        diff = y_true - y_pred
        squared_diff = diff**2
        ssd_distance = torch.sum(squared_diff)
